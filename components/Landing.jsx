...
const Landing = ({scrollTo}) => {
    ...
    useLayoutEffect(() => {
        ...
        tl.current.fromTo(q(".landing-desc"), {translateY: 20, autoAlpha: 0}, {autoAlpha: 1, translateY: 0, duration: 1, ease: "power2.out"}, 2.8);
    ...
    return (
        <div id="home" ref={el} className="w-full h-[115vh] relative bg-gradient-to-r from-blue-600 via-purple-600 to-indigo-700 min-h-screen">
            <div className="landing-title flex flex-col items-center absolute top-[25%] w-full text-neutral-100" data-speed="0.6">
                <h1 className="landing-name text-[clamp(60px,7vw,135px)] font-[600]">NFT 检测系统</h1>
                <h2 className="landing-sub pb-10 text-[clamp(20px,2vw,32px)] text-center px-4">
                    超越图像相似度：文本-视觉协同的NFT伪造品检测
                </h2>
                <p className="landing-desc text-[clamp(16px,1.5vw,20px)] text-center px-4 mb-8 text-neutral-200 max-w-3xl">
                ...
                <div className="flex gap-4 flex-wrap justify-center">
                    ...
                    <Link href="/detect">
                        <button className="rounded-xl px-8 py-3 text-neutral-100 font-[500] transition tracking-wide outline-none bg-blue-600 hover:bg-blue-700 shadow-lg hover:shadow-xl">
                            侵权检测
                        </button>
                    </Link>
                    <Link href="/register">
                        <button className="rounded-xl px-8 py-3 text-neutral-900 font-[500] transition tracking-wide outline-none bg-white hover:bg-neutral-100 shadow-lg hover:shadow-xl">
                            正版登记
                        </button>
                    </Link>
                </div>
